...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // GUARANTEE: Set hardcoded products immediately to ensure they always display
    ...
    console.log('SETTING GUARANTEED PRODUCTS:', guaranteedProducts.length);
    setProductsData(guaranteedProducts);
    setLoadingProducts(false);
    
    // Optional: Try to fetch from Supabase but don't block display
    const fetchSupabaseData = async () => {
      if (supabase) {
        try {
          const { data, error } = await supabase.from('products').select('*');
          if (data && data.length > 0) {
            console.log('Supabase data available, updating products:', data.length);
            setProductsData(data);
          } else {
            console.log('Using guaranteed hardcoded products');
          }
        } catch (err) {
          console.log('Supabase failed, using guaranteed products');
        }
      }
    };
    fetchSupabaseData();
