        ...
        <div className="max-w-[1400px] mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="hidden md:flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-xs font-bold transition-all"
            >
            ...
            {user ? (
              <div className="w-10 h-10 bg-gradient-to-br from-rose-500 to-rose-600 rounded-xl flex items-center justify-center font-bold shadow-lg shadow-rose-500/20">
                {user.user_metadata?.full_name?.charAt(0).toUpperCase() || 'U'}
              </div>
            ) : (
              <Link href="/#login" className="px-5 py-2.5 bg-rose-500 hover:bg-rose-600 rounded-xl text-sm font-bold transition-all shadow-lg shadow-rose-500/20">
                Masuk
              </Link>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-7 xl:col-span-8 space-y-12">
            ...
            <motion.div 
              ...
              className="relative overflow-hidden bg-gradient-to-br from-[#1e293b] to-[#0f172a] rounded-[40px] p-10 border border-white/5"
            >
              <div className="relative z-10 max-w-lg">
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-rose-500/10 border border-rose-500/20 rounded-full mb-6">
                  <span className="text-rose-500 text-[10px] font-black uppercase tracking-widest">Fitur Baru Toko Hijrah ✨</span>
                </div>
                <h2 className="text-4xl md:text-5xl font-black mb-6 leading-[1.1] tracking-tight">
                  Bingung Mau Masak Apa Hari Ini?
                </h2>
                <p className="text-white/60 text-lg mb-8 leading-relaxed">
                  Tenang! Sekarang ada <span className="text-white font-bold">Chef Virtual Hijrah</span> yang siap merekomendasikan resep praktis, ide olahan frozen food, hingga tips dapur seru. 100% gratis!
                </p>
                <div className="flex flex-wrap gap-4">
                  <button 
                    onClick={() => setInitialChefMessage("Halo Chef! Aku bingung mau masak apa hari ini, bisa beri saran?")}
                    className="px-8 py-4 bg-rose-500 hover:bg-rose-600 text-white rounded-2xl font-bold transition-all shadow-xl shadow-rose-500/25 flex items-center gap-3"
                  >
                    Mulai Tanya Chef 🧑‍🍳
                  </button>
                  <Link 
              ...
              <div className="absolute -right-20 -bottom-20 w-80 h-80 text-white/[0.03] rotate-12 pointer-events-none">
                <Info size={320} />
              </div>
            </motion.div>
            ...
            <section>
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-white/40 text-sm mt-1">Pilih bahan frozen favoritmu & langsung kreasikan resepnya</p>
                </div>
                <Link href="/#produk" className="text-rose-500 text-sm font-bold flex items-center gap-2 hover:gap-3 transition-all">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {products.map((p, idx) => (
                  <motion.div 
                    ...
                    className="group bg-[#111827] border border-white/5 rounded-3xl p-5 hover:border-rose-500/30 transition-all"
                  >
                    <div className="relative aspect-video rounded-2xl overflow-hidden mb-5">
                      ...
                      />
                      <div className="absolute top-3 left-3 px-2 py-1 bg-black/60 backdrop-blur-md rounded-lg text-[10px] font-bold uppercase tracking-wider text-white">
                        {p.category.replace('_', ' ')}
                      </div>
                    </div>
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-bold text-lg leading-tight">{p.name}</h4>
                      <span className="px-2 py-1 bg-white/5 rounded-lg text-[10px] text-white/40 font-bold">{p.weight || '500g'}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-auto">
                      <div>
                        <p className="text-[10px] text-white/30 uppercase font-bold tracking-widest">Harga</p>
                        <p className="text-rose-500 font-black">Rp {p.price.toLocaleString('id-ID')}</p>
                      </div>
                      <div className="flex gap-2">
                        <button 
                          onClick={() => askRecipe(p.name)}
                          className="px-4 py-2 bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 rounded-xl text-xs font-bold transition-all flex items-center gap-2 border border-rose-500/20"
                        >
                          Tanya Resep <Sparkles size={14} />
                        </button>
                        <button className="w-10 h-10 bg-white hover:bg-rose-500 text-black hover:text-white rounded-xl flex items-center justify-center transition-all shadow-lg active:scale-95">
          ...
          <div className="lg:col-span-5 xl:col-span-4 h-[800px] lg:h-[calc(100vh-160px)] sticky top-24">
