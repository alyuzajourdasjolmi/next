  ...
  return (
    <>
      ...
      <div className="flex flex-col h-full overflow-hidden">
        <div className="relative flex-1 min-h-[450px] bg-slate-100">
          {/* Search Overlay */}
          <div className="absolute top-4 left-4 right-4 z-[1000]">
            <form onSubmit={handleSearch} className="relative">
              <input
                ...
                className="w-full bg-white border-none rounded-2xl px-5 py-4 pr-12 text-[15px] shadow-[0_8px_30px_rgb(0,0,0,0.12)] focus:outline-none focus:ring-2 focus:ring-rose-500 transition-all font-medium text-slate-800"
              />
              <button 
                type="submit" 
                className="absolute right-2 top-1/2 -translate-y-1/2 p-2.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-colors"
                disabled={isSearching}
              >
                {isSearching ? <div className="animate-spin h-5 w-5 border-2 border-rose-500 rounded-full border-t-transparent"></div> : <Search size={20} />}
              </button>
            </form>
            
            {/* Search Results */}
            {searchResults.length > 0 && (
              <div className="mt-2 bg-white rounded-2xl shadow-[0_12px_40px_rgb(0,0,0,0.12)] border border-slate-100 max-h-60 overflow-y-auto overflow-x-hidden">
                {searchResults.map((result, i) => (
                  <button
                    key={i}
                    onClick={() => selectSearchResult(result)}
                    className="w-full text-left px-5 py-3.5 text-sm hover:bg-slate-50 border-b border-slate-50 last:border-0 transition-colors focus:bg-slate-50 focus:outline-none"
                  >
                    <p className="font-semibold text-slate-800 truncate text-[14px] mb-0.5">{result.name || result.display_name.split(',')[0]}</p>
                    <p className="text-[12px] text-slate-500 truncate">{result.display_name}</p>
                  </button>
                ))}
              </div>
            )}
          ...
          {!isMapLoaded && (
            <div className="absolute inset-0 z-[20] flex flex-col items-center justify-center bg-slate-50/90 backdrop-blur-sm">
              <div className="animate-spin rounded-full h-12 w-12 border-b-4 border-rose-500 mb-4"></div>
          ...
          <div className="absolute bottom-6 right-4 z-[1000] flex flex-col gap-3">
            <button 
              ...
              className="bg-white p-3.5 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.15)] hover:bg-slate-50 text-slate-700 hover:text-indigo-600 transition-all border border-slate-100 group"
              title="Ganti Tampilan Peta"
            >
              <Layers size={22} strokeWidth={2} className="group-hover:scale-110 transition-transform" />
            </button>
            <button 
              onClick={useCurrentLocation}
              className="bg-white p-3.5 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.15)] hover:bg-slate-50 text-slate-700 hover:text-rose-600 transition-all border border-slate-100 group"
              title="Gunakan Lokasi Saya"
            >
              <Navigation size={22} strokeWidth={2} className="group-hover:scale-110 transition-transform" />
            </button>
          </div>
          
          {/* Overlay text indicating to drag map */}
          {isMapLoaded && (
            <div className="absolute top-[80px] left-1/2 -translate-x-1/2 z-[1000] pointer-events-none">
              <div className="bg-slate-900/70 backdrop-blur-md text-white px-4 py-1.5 rounded-full text-xs font-medium shadow-lg">
        ...
        <div className="p-6 bg-white space-y-5 z-[20] border-t border-slate-100 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] rounded-t-3xl -mt-4 relative">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 bg-rose-50 rounded-2xl flex items-center justify-center shrink-0 mt-0.5 text-rose-600 border border-rose-100">
              <MapPin size={24} strokeWidth={2.5} />
            </div>
            <div className="flex-1">
              <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mb-1.5 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                Lokasi Pengiriman
              </p>
              <p className="text-[15px] text-slate-800 font-semibold leading-relaxed line-clamp-2">
          ...
          <div className="flex gap-3 pt-2">
            <button 
              onClick={onCancel}
              className="px-6 py-4 rounded-2xl text-[15px] font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-all focus:ring-4 focus:ring-slate-100 focus:outline-none"
            >
              Batal
            </button>
            <button 
              ...
              disabled={isDragging}
              className={`flex-1 px-6 py-4 rounded-2xl text-[15px] font-bold text-white transition-all flex items-center justify-center gap-2 ${
                isDragging 
            ...
            >
              <Check size={20} strokeWidth={2.5} /> Konfirmasi Lokasi
            </button>
          </div>
        </div>
      </div>
